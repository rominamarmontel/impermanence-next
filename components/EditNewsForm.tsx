...
const EditNewsForm = ({ news }: { news: TNews }) => {
  ...
  useEffect(() => {
    const initValues = () => {
      setPostTitle(news.postTitle || '')
      setPost(news.post || '')
      setImageUrl(news.imageUrl || '')
      setPublicId(news.publicId || '')
    }
    initValues()
  }, [news.postTitle, news.post, news.imageUrl, news.publicId])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXTAUTH_URL}/api/news/${news._id}`,
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
    ...
    <div className={styles.Form}>
      <div className={styles.FormContent}>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-2 ">
          ...
          <button className="primary-btn mt-10">Update News</button>
        </form>
      </div>
    </div>
  )
